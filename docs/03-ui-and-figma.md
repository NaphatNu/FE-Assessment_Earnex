...
## การเก็บค่าจาก Figma ลงโค้ด

รอบนี้ค่าทุกอย่างถูกดึงจากไฟล์ Figma โดยตรง (ไม่ได้กะจากภาพ) แล้ววางไว้ที่เดียวใน
[`lib/src/ui/theme/tokens.dart`](../lib/src/ui/theme/tokens.dart) ซึ่งสะท้อน
variable collection ทั้งสี่ของหน้า Design Foundation แบบหนึ่งต่อหนึ่ง

| ใน Figma | ในโค้ด |
|---|---|
| `Primitive` + `Semantic` (สี) | `AppColors` |
| `Spacing` (`spacing/*`) | `AppSpacing` |
| `Spacing` (`radius/*`) | `AppRadius` |
| `Typography` + text style | `AppText` |

**ฟอนต์** — ดีไซน์ใช้ Inter ทั้งไฟล์ โปรเจกต์จึง bundle Inter 4 น้ำหนัก
(400/500/600/700) ไว้ที่ `assets/fonts/` ก่อนหน้านี้ `ThemeData` ประกาศ
`fontFamily: 'Inter'` ไว้แต่ไม่มีไฟล์ฟอนต์จริง Flutter จึง fallback ไปใช้ Roboto
เงียบ ๆ ทำให้ความกว้างตัวอักษรและตำแหน่งตัดบรรทัดเพี้ยนจากดีไซน์ทั้งหน้า

**ไอคอน** — ไอคอนทุกตัวถูก export เป็น PNG @4x จาก node ในไฟล์ Figma ลง
`assets/icons/` แทนการเดาด้วย `Icons.*` ของ Material เพราะรูปทรงไม่ตรงกัน
(เช่น ไอคอน filter, ไอคอนวงแหวนรอบ avatar, ลูกศร chevron)

**สิ่งที่ยังเป็นการประมาณ** — มีสองจุดเท่านั้น และทั้งสองจุดจงใจ

1. **แสงเรืองสีทองบนการ์ด** ดีไซน์ใช้วงรี `#D89F00` ที่ผ่าน layer blur 200px
   การ blur ขนาดนั้นหนึ่งครั้งต่อการ์ดหนึ่งใบใน list ที่เลื่อนได้คือค่าใช้จ่ายจริง
   จึงวาดเป็น `RadialGradient` ที่ให้ผลตาเห็นเท่ากันแทน
2. **กราฟ sparkline บนการ์ด** ข้อมูลไม่มี time series — ดูหัวข้อ ① ด้านบน
   หลักการเดียวกัน: ไม่มีข้อมูลจริง ก็ไม่ควรแสดงเป็น "ตัวเลข" เส้นกราฟจึงถูก
   สร้างจาก seed ของ trader id (การ์ดใบเดิมได้เส้นเดิมเสมอ) และย้อมสีตาม
   เครื่องหมายของ PnL ส่วนตัวเลขทุกตัวบนการ์ดยังมาจาก mock data ล้วน ๆ
